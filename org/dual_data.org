...
#+begin_src ipython :results raw drawer :exports both :async t :session dual_data
  ...
  concat_fig('summary', figlist, dim=[3, 1], size=figsize, VLINE=0)
...
:results:
# Out[13]:
[[file:./obipy-resources/qt4s8z.png]]
:end:
...
#+begin_src ipython :results raw drawer :exports both :async t :session dual_data
  mouse = 'JawsM18'
  run_get_overlap(mouse=mouse, features='distractor', task='DualGo', day='first', method='bolasso', pval=0.001, balance=0)
  run_get_overlap(mouse=mouse, features='distractor', task='DualGo', day='last', method='bolasso', pval=0.001, balance=0)
#+end_src

#+RESULTS:
:results:
# Out[19]:
[[file:./obipy-resources/Ah6RLe.png]]
:end:
