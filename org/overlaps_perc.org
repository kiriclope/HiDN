...
: scores (6, 3, 6, 5, 84) (6, 3, 6, 5, 84) (6, 3, 6, 5, 84)
...
#+begin_src ipython
  ...
  pkl_save(overlaps, '%s_overlaps' % options['mouse'], path="../data/%s/" % options['mouse'])
  ...
  print(coefs.shape)
  pkl_save(coefs, '%s_coefs' % options['mouse'], path="../data/%s/" % options['mouse'])
#+end_src
...
#+RESULTS:
[[file:./.ob-jupyter/fb7c14ea2eb664497f49fbc629d81e060471a619.png]]
...
#+RESULTS:
[[file:./.ob-jupyter/76449599d80c2fdbc5f3916ba50adddd400309cf.png]]
...
#+RESULTS:
[[file:./.ob-jupyter/5631c435532107afa0f5b205194a541978586199.png]]
...
#+begin_src ipython
    ...
    sample_ci = []
    for i in range(options['n_days']):
        sample_epoch = avg_epochs(-overlaps_sample[i][32:] + overlaps_sample[i][:32], **options) / 2.0
        sample_avg.append(sample_epoch.mean(0))
        sample_ci.append(get_bootstrap_ci(sample_epoch))
    ...
    dist_ci = []
    for i in range(options['n_days']):
        dist_epoch = avg_epochs(overlaps_dist[i][32:] + overlaps_dist[i][:32], **options) / 2.0 * 1000
        dist_avg.append(dist_epoch.mean(0))
        dist_ci.append(get_bootstrap_ci(dist_epoch))
    ...
    choice_ci = []
    for i in range(options['n_days']):
        choice_epoch = avg_epochs(overlaps_choice[i][32:] + overlaps_choice[i][:32], **options) / 2.0 * 1000
        choice_avg.append(choice_epoch.mean(0))
        choice_ci.append(get_bootstrap_ci(choice_epoch))
...
#+RESULTS:
[[file:./.ob-jupyter/b9a9fc94ce719404ab577df2f0e4a246073aed90.png]]
