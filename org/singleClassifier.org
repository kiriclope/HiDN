...
:GPTEL_TOOLS:
:GPTEL_BOUNDS: ((response (19511 21605) (21609 22006) (22010 22081) (22085 22218) (22427 22428) (22436 22474) (22482 22512) (22520 22565) (22573 22629) (22637 22669) (22677 22679) (22687 22690) (22698 23081) (23106 23439) (23707 23950) (23963 24233) (24789 24865) (24869 24973) (25267 25323) (25328 25384) (27389 27555) (27829 28030)))
:END:
#+STARTUP: fold
#+PROPERTY: header-args:jupyter-python :results both :exports both :async yes :session singleClass :kernel dual :output-dir ./figures/singleClass :file (lc/org-babel-tangle-figure-filename)
...
#+RESULTS:
: b85f7155-2970-461f-8e78-284009dd68b7
...
#+RESULTS:
: e79ccce6-86c9-482d-8b23-1640212ccd56
...
def _get_coefs(estimator):
    # SlidingEstimator: estimator.estimators_ is a list over time
    # each entry is a fitted sklearn estimator
    coefs = []
    for est_t in estimator.estimators_:
        if hasattr(est_t, "coef_"):
            coefs.append(est_t.coef_)
        elif hasattr(est_t, "best_estimator_") and hasattr(est_t.best_estimator_, "coef_"):
            coefs.append(est_t.best_estimator_.coef_)
        else:
            raise AttributeError(f"Cannot find coef_ in {type(est_t)}")
    return np.stack(coefs, axis=0)  # (n_times, n_classes or 1, n_features)
...
def cross_val_score(
    ...
    if mode == "sliding":
        estimator = SlidingEstimator(clf, scoring=scoring, n_jobs=n_jobs, verbose=False)
    else:
        estimator = GeneralizingEstimator(clf, scoring=scoring, n_jobs=n_jobs, verbose=False)
...
def smooth_and_bin2(X, sigma=1.0):
    ...
    T2 = T // 2
    X_s = X_s[..., :2*T2]
    X_b = X_s.reshape(*X_s.shape[:-1], T2, 2).mean(-1)
    return X_b
#+end_src
...
for mouse in options['mice']:
    ...
    Z_sample, y_sample, w_sample = cross_val_score(X_pca, y_pca, clf=None, label_col="sample_odor", cv=folds, scoring="accuracy", mode="generalizing")
    Z_choice, y_choice, w_choice = cross_val_score(X_pca, y_pca, clf=None, label_col="choice", cv=folds, scoring="accuracy", mode="generalizing")
    Z_test, y_test, w_test = cross_val_score(X_pca, y_pca, clf=None, label_col="test_odor", cv=folds, scoring="accuracy", mode="generalizing")
...
#+RESULTS:
: saving to ../data/class//single_traj_TEST.pkl
: saving to ../data/class//single_labels_TEST.pkl
...
#+RESULTS:
: saving to ../data/class//single_coefs_TEST.pkl
...
#+RESULTS:
: c1954f7d-6d53-432c-9778-d57f8a5622f8
...
#+RESULTS:
: loading from ../data/class//single_traj_TEST.pkl
: loading from ../data/class//single_labels_TEST.pkl
: loading from ../data/class//single_coefs_TEST.pkl

#+begin_src jupyter-python
X_epoch = X_single[..., options['bins_RWD2'] //2, :].mean(-2)
print(X_single.shape, X_epoch.shape)
#+end_src

#+RESULTS:
: (9216, 3, 42, 42) (9216, 3, 42)
...
idx_correct = (y_single.learning=='Naive') & (y_single.performance==0)
idx_mouse = True
if i_mouse !=-1:
...
for i in range(len(pair)):
    mask = (y_single.tasks==y_single.tasks.unique()[i]) & (y_single.laser==laser) & idx_mouse & idx_correct & (y_single.odr_perf==1)
    X_sel = X_epoch[mask]
...
#+RESULTS:
[[file:./figures/singleClass/figure_52.png]]
...
#+RESULTS:
: 85a8f267-5177-4405-b3ec-21286e47a61c
