...
DATA: FEATURES odr_choice TASK DPA TRIALS incorrect DAYS first LASER 0
multiple days, discard 0 first 3 middle 0
X_S1 (10, 184, 84) X_S2 (96, 184, 84)
X_B (106, 184, 84) y_B (106,) [1.] ['DualGo' 'DualNoGo' 'DPA'] [ 0. nan]
DATA: FEATURES odr_choice TASK Dual TRIALS correct DAYS first LASER 0
multiple days, discard 0 first 3 middle 0
...
Loading files from /home/leon/dual_task/dual_data/data/JawsM01
X_days (768, 184, 84) y_days (768, 12)
DATA: FEATURES odr_choice TASK DPA TRIALS incorrect DAYS last LASER 0
multiple days, discard 0 first 3 middle 0
X_S1 (1, 184, 84) X_S2 (32, 184, 84)
X_B (33, 184, 84) y_B (33,) [1.] ['DualNoGo' 'DPA'] [ 0. nan]
DATA: FEATURES odr_choice TASK Dual TRIALS correct DAYS last LASER 0
multiple days, discard 0 first 3 middle 0
...
df_dist['pair'] = df_dist['response'].apply(lambda x: 0 if (('rej' in x) or ('fa' in x)) else 1)
 ...
 : Cell In[1], line 1
 : ----> 1 df_dist['performance'] = df_dist['response'].apply(lambda x: 0 if 'incorrect' in x else 1)
 :       2 df_dist['pair'] = df_dist['response'].apply(lambda x: 0 if (('rej' in x) or ('fa' in x)) else 1)
 :
...
#+begin_src ipython
if len(days)>3:
    name = 'df_odr_choice_%s_days' % dum
elif len(days)==2:
    name = 'df_odr_choice_%s_early_late' % dum
else:
    name = 'df_odr_choice_%s' % dum
...
#+RESULTS:
: saving to ../data/mice/overlaps/df_odr_choice_overlaps_loocv_early_late.pkl
...
#+RESULTS:
: loading from ../data/mice/overlaps/df_odr_choice_overlaps_loocv_early_late.pkl
...
#+begin_src ipython
options['T_WINDOW'] = 0.0
for epoch in ['DIST', 'DIST_MD', 'MD', 'CUE', 'RWD' ,'LD', 'CHOICE']:
...
Index(['index', 'sample_odor', 'dist_odor', 'test_odor', 'tasks', 'response',
       ...
       'overlaps_DIST', 'overlaps_DIST_ED', 'overlaps_DIST_DIST',
       'overlaps_DIST_MD', 'overlaps_DIST_CUE', 'overlaps_DIST_RWD',
       'overlaps_DIST_LD', 'overlaps_DIST_MD_ED', 'overlaps_DIST_MD_DIST',
...
:END:
 ...
 #+begin_src ipython
n_ = len(options['days'])
fig, ax = plt.subplots(nrows=n_, ncols=3, figsize=(3*width, n_*height), sharex=True, sharey=True)
...
for i in range(len(options['days'])):
    for j in range(len(tasks)):
        df_ = df[df.tasks==tasks[j]]

        plot_overlaps(df_[(df_.pair==1)], options['days'][i], epoch, ax[i][j], title='Sample', y0=0., ls='-', label='Pair', colors=[colors[j]])
        plot_overlaps(df_[(df_.pair==0)], options['days'][i], epoch, ax[i][j], title='Sample', y0=0.,ls='--', label='Unpair', colors=[colors[j]])

        if j==0:
            ax[i][j].set_ylabel('%s \n GNG Overlap' % options['days'][i])
        else:
            ax[i][j].set_ylabel('GNG Overlap')

        if i == 0:
            ax[i][j].set_title(tasks[j])

        ax[i][j].set_xlim([2, 12])

plt.savefig('figures/icrm/dist_overlaps_%s_licks.svg' % epoch, dpi=300)
plt.show()
#+end_src

#+RESULTS:
[[./figures/dist_overlaps/figure_35.png]]
...
plot_overlaps_mat(df, 'first', vmin=-2, vmax=2, title='Dist')
#+end_src

#+RESULTS:
[[./figures/dist_overlaps/figure_46.png]]
