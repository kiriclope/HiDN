#+STARTUP: fold
#+PROPERTY: header-args:jupyter-python :results both :exports both :async yes :session singleFlow :kernel dual :output-dir ./figures/singleFlow :file (lc/org-babel-tangle-figure-filename)
...
#+RESULTS:
: 96fdd300-2c7a-4d47-994d-3eb1379e1898
...
**** 1. Masks and basic settings

#+begin_src jupyter-python
# ---- settings for Fig 2 ----
x_pc = 0   # PC1 ~ sample / A-B overlap
y_pc = 1   # PC2 ~ choice overlap
z_lim = 20
sigma_traj = 1.0
grid_size = 80
...
print("Naive DPA correct:", mask_naive_dpa.sum())
print("Expert DPA correct:", mask_expert_dpa.sum())
#+end_src

#+RESULTS:
: Naive DPA correct: 731
: Expert DPA correct: 700

---

**** 2. Helpers specialized for your notebook
...
#+begin_src jupyter-python
def get_xy_from_mask(X, mask, x_pc=0, y_pc=1):
    Xsel = X[mask]
    x = Xsel[:, x_pc, :]
    y = Xsel[:, y_pc, :]
    return x, y
...
#+begin_src jupyter-python
def get_mean_traj_by_sample(X, y, mask, x_pc=0, y_pc=1):
    out = {}
    yy = y.loc[mask].reset_index(drop=True)
    XX = X[mask]

    for samp in sorted(yy["sample_odor"].unique()):
        m = (yy["sample_odor"] == samp).values
        if m.sum() == 0:
            continue
        out[samp] = {
            "x": np.nanmean(XX[m, x_pc, :], axis=0),
            "y": np.nanmean(XX[m, y_pc, :], axis=0),
            "n": m.sum()
        }
    return out
#+end_src
...
#+begin_src jupyter-python
def get_velocity_field(x, y, sigma=1.0, grid_size=80, z_lim=20):
    # smooth trajectories over time
    x_s = gaussian_filter(x, sigma=(0, sigma))
    y_s = gaussian_filter(y, sigma=(0, sigma))

    # velocities
    u = np.gradient(x_s, axis=1)
    v = np.gradient(y_s, axis=1)

    # interpolate to grid using your notebook helper
    xi, yi, ui, vi = create_field(x_s, y_s, u, v, grid_size=grid_size, z_lim=z_lim)
    speed = np.sqrt(ui**2 + vi**2)
    return xi, yi, ui, vi, speed
#+end_src
...
#+begin_src jupyter-python
def plot_overlap_map_panel(
    ax, x, y, trajs=None, title="",
    z_lim=20, sigma=1.0, grid_size=80,
    vmin=None, vmax=None, cmap="coolwarm_r"
):
    xi, yi, ui, vi, speed = get_velocity_field(
        x, y, sigma=sigma, grid_size=grid_size, z_lim=z_lim
    )
    ...
    if trajs is not None:
        colors = {0: "#2aa198", 1: "#3b2f92"}  # sample A/B
        for k, d in trajs.items():
            col = colors.get(k, "k")
            ax.plot(d["x"], d["y"], color=col, lw=3)
            ax.scatter(d["x"][0], d["y"][0], s=180, color="white", zorder=5)
            ax.scatter(d["x"][-1], d["y"][-1], s=60, color=col, edgecolor="white", linewidth=0.5, zorder=6)

    ax.axhline(0, ls="--", color="w", alpha=0.7)
    ax.axvline(0, ls="--", color="w", alpha=0.7)
...
**** 3. Fig. 2E — naive vs expert overlap maps

#+begin_src jupyter-python
xN, yN = get_xy_from_mask(X_single, mask_naive_dpa, x_pc=x_pc, y_pc=y_pc)
xE, yE = get_xy_from_mask(X_single, mask_expert_dpa, x_pc=x_pc, y_pc=y_pc)

trajN = get_mean_traj_by_sample(X_single, y_single, mask_naive_dpa, x_pc=x_pc, y_pc=y_pc)
trajE = get_mean_traj_by_sample(X_single, y_single, mask_expert_dpa, x_pc=x_pc, y_pc=y_pc)

_, _, _, _, sN = get_velocity_field(xN, yN, sigma=sigma_traj, grid_size=grid_size, z_lim=z_lim)
_, _, _, _, sE = get_velocity_field(xE, yE, sigma=sigma_traj, grid_size=grid_size, z_lim=z_lim)

vmin = np.nanpercentile(np.r_[sN.ravel(), sE.ravel()], 5)
vmax = np.nanpercentile(np.r_[sN.ravel(), sE.ravel()], 95)

fig, ax = plt.subplots(1, 2, figsize=(2.3*width, 1.15*2*height), sharex=True, sharey=True)

im, _ = plot_overlap_map_panel(
    ax[0], xN, yN, trajs=trajN,
    title="Naive", z_lim=z_lim, sigma=sigma_traj, grid_size=grid_size,
    vmin=vmin, vmax=vmax
)

plot_overlap_map_panel(
    ax[1], xE, yE, trajs=trajE,
    title="Expert", z_lim=z_lim, sigma=sigma_traj, grid_size=grid_size,
    vmin=vmin, vmax=vmax
)
...
plt.tight_layout()
plt.savefig("./figures/single/fig2E_overlap_maps_dpa.svg")
plt.show()
#+end_src

#+RESULTS:
[[file:./figures/singleFlow/figure_30.png]]

If this still looks too noisy, I’d recommend replacing pooled single-trial fields with /mouse-averaged condition trajectories/. I can give that version too if needed.

---

**** 4. Fig. 2F — per-mouse delay centroid shift

This is the key quantification.

#+begin_src jupyter-python
def get_delay_centroid_per_mouse(X, y, stage, bins, x_pc=0, y_pc=1):
    rows = []
    for mouse in sorted(y["mouse"].unique()):
        mask = (
            (y["mouse"] == mouse)
            & (y["learning"] == stage)
            & (y["tasks"] == "DPA")
        ...
        Xsel = X[mask]
        x_delay = np.nanmean(Xsel[:, x_pc, :][:, bins])
        y_delay = np.nanmean(Xsel[:, y_pc, :][:, bins])
...
#+begin_src jupyter-python
dfN = get_delay_centroid_per_mouse(X_single, y_single, "Naive", options["bins_DELAY"], x_pc=x_pc, y_pc=y_pc)
dfE = get_delay_centroid_per_mouse(X_single, y_single, "Expert", options["bins_DELAY"], x_pc=x_pc, y_pc=y_pc)

df_delay = pd.concat([dfN, dfE], ignore_index=True)
display(df_delay)
#+end_src

#+RESULTS:
:RESULTS:
|    | mouse   | stage  | x_delay   | y_delay   | n_trials |
|----+---------+--------+-----------+-----------+----------|
| 0  | ACCM03  | Naive  | 2.381178  | -2.199577 | 133      |
| 1  | ACCM04  | Naive  | -0.249896 | -0.589215 | 108      |
| 2  | ChRM04  | Naive  | 2.217744  | -5.349425 | 80       |
| 3  | ChRM23  | Naive  | 3.891802  | 1.200714  | 63       |
| 4  | JawsM01 | Naive  | -0.773826 | -1.142971 | 75       |
| 5  | JawsM06 | Naive  | 0.063417  | -0.084766 | 60       |
| 6  | JawsM12 | Naive  | -0.287756 | 1.209820  | 62       |
| 7  | JawsM15 | Naive  | 2.553139  | -4.170303 | 70       |
| 8  | JawsM18 | Naive  | -1.026690 | -2.387863 | 80       |
| 9  | ACCM03  | Expert | 1.473150  | -3.387016 | 122      |
| 10 | ACCM04  | Expert | -0.299985 | -0.239135 | 93       |
| 11 | ChRM04  | Expert | 1.409354  | -6.380281 | 88       |
| 12 | ChRM23  | Expert | 3.532456  | 1.196401  | 48       |
| 13 | JawsM01 | Expert | -1.735644 | -0.853580 | 31       |
| 14 | JawsM06 | Expert | 0.375156  | 0.038399  | 84       |
| 15 | JawsM12 | Expert | -0.077696 | -0.000667 | 50       |
| 16 | JawsM15 | Expert | 2.596735  | -4.760477 | 89       |
| 17 | JawsM18 | Expert | 1.260464  | -0.637317 | 95       |
:END:



#+begin_src jupyter-python
wide_x = df_delay.pivot(index="mouse", columns="stage", values="x_delay").dropna()
wide_y = df_delay.pivot(index="mouse", columns="stage", values="y_delay").dropna()

px = wilcoxon(wide_x["Naive"], wide_x["Expert"]).pvalue
py = wilcoxon(wide_y["Naive"], wide_y["Expert"]).pvalue

fig, ax = plt.subplots(1, 2, figsize=(2*width, height))

for mouse in wide_x.index:
    ax[0].plot([0, 1], [wide_x.loc[mouse, "Naive"], wide_x.loc[mouse, "Expert"]], color="0.7", lw=1)
    ax[0].scatter([0, 1], [wide_x.loc[mouse, "Naive"], wide_x.loc[mouse, "Expert"]], s=60, zorder=3)

for mouse in wide_y.index:
    ax[1].plot([0, 1], [wide_y.loc[mouse, "Naive"], wide_y.loc[mouse, "Expert"]], color="0.7", lw=1)
    ax[1].scatter([0, 1], [wide_y.loc[mouse, "Naive"], wide_y.loc[mouse, "Expert"]], s=60, zorder=3)

ax[0].axhline(0, ls="--", color="k", alpha=0.5)
ax[1].axhline(0, ls="--", color="k", alpha=0.5)

ax[0].set_xticks([0, 1]); ax[0].set_xticklabels(["Naive", "Expert"])
ax[1].set_xticks([0, 1]); ax[1].set_xticklabels(["Naive", "Expert"])

ax[0].set_ylabel("Delay centroid\nA/B overlap")
ax[1].set_ylabel("Delay centroid\nChoice overlap")

ax[0].set_title(f"Sample axis\np = {px:.3g}")
ax[1].set_title(f"Choice axis\np = {py:.3g}")

plt.tight_layout()
plt.savefig("./figures/single/fig2F_delay_centroids_dpa.svg")
plt.show()
#+end_src

#+RESULTS:
[[file:./figures/singleFlow/figure_33.png]]

Optional better metric: fraction of delay occupancy in lower half-plane.

#+begin_src jupyter-python
def get_lower_half_fraction_per_mouse(X, y, stage, bins, y_pc=1):
    rows = []
    for mouse in sorted(y["mouse"].unique()):
        mask = (
            (y["mouse"] == mouse)
        ...
        rows.append({"mouse": mouse, "stage": stage, "frac_lower": frac})
    return pd.DataFrame(rows)
#+end_src

#+RESULTS:

That could be panel F right instead of centroid y if you prefer a more interpretable “no-lick occupancy” metric.

---

**** 5. Fig. 2G — geometry vs behavior correlation

For behavior:
- DPA improvement = =performance= on DPA trials
- GNG improvement = =odr_perf= on DualGo + DualNoGo trials

#+begin_src jupyter-python
def get_perf_per_mouse(y, stage, mode="dpa"):
    rows = []
    for mouse in sorted(y["mouse"].unique()):
        if mode == "dpa":
            mask = (
            ...
            perf = y.loc[mask, "performance"].mean() if mask.sum() else np.nan

        elif mode == "gng":
            mask = (
                (y["mouse"] == mouse)
                & (y["learning"] == stage)
                & (y["tasks"].isin(["DualGo", "DualNoGo"]))
                & (y["laser"] == 0)
            )
            perf = y.loc[mask, "odr_perf"].mean() if mask.sum() else np.nan
...
#+begin_src jupyter-python
# geometry shift from panel F
geom_wide = df_delay.pivot(index="mouse", columns="stage", values="y_delay").dropna()
dgeom = (geom_wide["Expert"] - geom_wide["Naive"]).rename("d_choice_geom")

# behavior shifts
dpa_df = pd.concat([
    get_perf_per_mouse(y_single, "Naive", mode="dpa"),
    get_perf_per_mouse(y_single, "Expert", mode="dpa")
], ignore_index=True)

gng_df = pd.concat([
    get_perf_per_mouse(y_single, "Naive", mode="gng"),
    get_perf_per_mouse(y_single, "Expert", mode="gng")
], ignore_index=True)

dpa_w = dpa_df.pivot(index="mouse", columns="stage", values="perf").dropna()
gng_w = gng_df.pivot(index="mouse", columns="stage", values="perf").dropna()

ddpa = (dpa_w["Expert"] - dpa_w["Naive"]).rename("d_dpa")
dgng = (gng_w["Expert"] - gng_w["Naive"]).rename("d_gng")

df_corr = pd.concat([dgeom, ddpa, dgng], axis=1).dropna()
display(df_corr)
#+end_src
...
for axi, col, title in zip(ax, ["d_dpa", "d_gng"], ["DPA", "GNG"]):
    r, p = pearsonr(df_corr["d_choice_geom"], df_corr[col])
    ...
    sns.regplot(
        ...
        y=col,
        ax=axi,
        scatter_kws=dict(s=80, edgecolor="black", linewidth=0.3),
        line_kws=dict(color="k", ls="--")
    )

    axi.axhline(0, ls="--", color="0.5")
    axi.axvline(0, ls="--", color="0.5")
    axi.set_xlabel(r"$\Delta$ choice overlap")
    axi.set_ylabel(r"$\Delta$ performance")
    axi.set_title(f"{title}\nr = {r:.2f}, p = {p:.3f}")

plt.tight_layout()
plt.savefig("./figures/single/fig2G_geometry_behavior_corr_dpa.svg")
plt.show()
#+end_src
...
Cell In[66], line 6
      3 for axi, col, title in zip(ax, ["d_dpa", "d_gng"], ["DPA", "GNG"]):
      4     r, p = pearsonr(df_corr["d_choice_geom"], df_corr[col])
----> 6     sns.regplot(
      7         data=df_corr,
      8         x="d_choice_geom",
      9         y=col,
     10         ax=axi,
     11         scatter_kws=dict(s=80, edgecolor="black", linewidth=0.3),
     12         line_kws=dict(color="k", ls="--")
     13     )
     15     axi.axhline(0, ls="--", color="0.5")
     16     axi.axvline(0, ls="--", color="0.5")

File ~/mambaforge/envs/dual/lib/python3.14/site-packages/seaborn/regression.py:775, in regplot(data, x, y, x_estimator, x_bins, x_ci, scatter, fit_reg, ci, n_boot, units, seed, order, logistic, lowess, robust, logx, x_partial, y_partial, truncate, dropna, x_jitter, y_jitter, label, color, marker, scatter_kws, line_kws, ax)
...
File ~/mambaforge/envs/dual/lib/python3.14/site-packages/seaborn/regression.py:381, in _RegressionPlotter.plot(self, ax, scatter_kws, line_kws)
    379 # Draw the constituent plots
    380 if self.scatter:
--> 381     self.scatterplot(ax, scatter_kws)
    383 if self.fit_reg:
    384     self.lineplot(ax, line_kws)

File ~/mambaforge/envs/dual/lib/python3.14/site-packages/seaborn/regression.py:411, in _RegressionPlotter.scatterplot(self, ax, kws)
    408         kws.setdefault("alpha", .8)
    410     x, y = self.scatter_data
--> 411     ax.scatter(x, y, **kws)
    412 else:
    413     # TODO abstraction
    414     ci_kws = {"color": kws["color"]}

File ~/mambaforge/envs/dual/lib/python3.14/site-packages/matplotlib/_api/deprecation.py:453, in make_keyword_only.<locals>.wrapper(*args, **kwargs)
...
File ~/mambaforge/envs/dual/lib/python3.14/site-packages/matplotlib/__init__.py:1524, in _preprocess_data.<locals>.inner(ax, data, *args, **kwargs)
...
File ~/mambaforge/envs/dual/lib/python3.14/site-packages/matplotlib/axes/_axes.py:4924, in Axes.scatter(self, x, y, s, c, marker, cmap, norm, vmin, vmax, alpha, linewidths, edgecolors, colorizer, plotnonfinite, **kwargs)
   4921 if linewidths is not None:
   4922     kwargs.update({'linewidths': linewidths})
-> 4924 kwargs = cbook.normalize_kwargs(kwargs, mcoll.Collection)
   4925 # re direct linewidth and edgecolor so it can be
   4926 # further processed by the rest of the function
   4927 linewidths = kwargs.pop('linewidth', None)

File ~/mambaforge/envs/dual/lib/python3.14/site-packages/matplotlib/cbook.py:1800, in normalize_kwargs(kw, alias_mapping)
...
#+end_example
[[file:./figures/singleFlow/figure_37.png]]
:END:

This is now much closer to your existing opto correlation panels, just applied to learning.

---

**** 6. Fig. 2H — correct vs error trajectories on unpaired DPA

This is the best candidate for the error panel.

From your coding:
- unpaired DPA are likely =odor_pair in [1, 3]=, i.e. AD and BC.
...
mask_unpaired_naive = (
    ...
    & (y_single["tasks"] == "DPA")
    & (y_single["laser"] == 0)
    & (y_single["odor_pair"].isin([1, 3]))
)

mask_corr = mask_unpaired_naive & (y_single["performance"] == 1)
mask_err  = mask_unpaired_naive & (y_single["performance"] == 0)

print("Correct unpaired DPA:", mask_corr.sum())
print("Error unpaired DPA:", mask_err.sum())
#+end_src

#+RESULTS:
: Correct unpaired DPA: 283
: Error unpaired DPA: 245

If this gives enough error trials, continue:

#+begin_src jupyter-python
def get_mean_sem_traj(X, mask, x_pc=0, y_pc=1):
    Xsel = X[mask]
    x = Xsel[:, x_pc, :]
    y = Xsel[:, y_pc, :]
    ...
    return {
        ...
        "x_sem": np.nanstd(x, axis=0) / np.sqrt(max(1, x.shape[0])),
        "y_sem": np.nanstd(y, axis=0) / np.sqrt(max(1, y.shape[0])),
        "n": x.shape[0]
    }
#+end_src
...
#+begin_src jupyter-python
traj_corr = get_mean_sem_traj(X_single, mask_corr, x_pc=x_pc, y_pc=y_pc)
traj_err  = get_mean_sem_traj(X_single, mask_err,  x_pc=x_pc, y_pc=y_pc)
...
ax[0].plot(traj_corr["x_mean"], traj_corr["y_mean"], color="#1f77b4", lw=3, label=f"Correct (n={traj_corr['n']})")
ax[0].plot(traj_err["x_mean"],  traj_err["y_mean"],  color="#d62728", lw=3, label=f"Error (n={traj_err['n']})")
ax[0].scatter(traj_corr["x_mean"][0], traj_corr["y_mean"][0], s=140, color="white", zorder=5)
ax[0].scatter(traj_err["x_mean"][0],  traj_err["y_mean"][0],  s=140, color="white", zorder=5)
ax[0].axhline(0, ls="--", color="k", alpha=0.5)
ax[0].axvline(0, ls="--", color="k", alpha=0.5)
ax[0].set_xlabel("A/B Overlap")
ax[0].set_ylabel("Choice Overlap")
ax[0].set_title("Unpaired DPA trajectories")
ax[0].legend(frameon=False)
ax[0].set_aspect("equal")

# choice coordinate over time
ax[1].plot(xtime, traj_corr["y_mean"], color="#1f77b4", lw=3, label="Correct")
ax[1].fill_between(
    xtime,
    traj_corr["y_mean"] - traj_corr["y_sem"],
    traj_corr["y_mean"] + traj_corr["y_sem"],
    color="#1f77b4", alpha=0.2
)

ax[1].plot(xtime, traj_err["y_mean"], color="#d62728", lw=3, label="Error")
ax[1].fill_between(
    xtime,
    traj_err["y_mean"] - traj_err["y_sem"],
    traj_err["y_mean"] + traj_err["y_sem"],
...
ax[1].set_title("Late drift toward lick sector")
ax[1].legend(frameon=False)

plt.tight_layout()
plt.savefig("./figures/single/fig2H_correct_vs_error_unpaired.svg")
plt.show()
#+end_src

#+RESULTS:
[[file:./figures/singleFlow/figure_40.png]]

If you want the 2H panel to match 2E more closely, use occupancy maps instead:

#+begin_src jupyter-python
xC, yC = get_xy_from_mask(X_single, mask_corr, x_pc=x_pc, y_pc=y_pc)
xR, yR = get_xy_from_mask(X_single, mask_err,  x_pc=x_pc, y_pc=y_pc)

_, _, _, _, sC = get_velocity_field(xC, yC, sigma=sigma_traj, grid_size=grid_size, z_lim=z_lim)
_, _, _, _, sR = get_velocity_field(xR, yR, sigma=sigma_traj, grid_size=grid_size, z_lim=z_lim)

vmin = np.nanpercentile(np.r_[sC.ravel(), sR.ravel()], 5)
vmax = np.nanpercentile(np.r_[sC.ravel(), sR.ravel()], 95)

fig, ax = plt.subplots(1, 2, figsize=(2.2*width, 1.1*height), sharex=True, sharey=True)

im, _ = plot_overlap_map_panel(ax[0], xC, yC, title=f"Correct\nn={mask_corr.sum()}", z_lim=z_lim, vmin=vmin, vmax=vmax)
plot_overlap_map_panel(ax[1], xR, yR, title=f"Error\nn={mask_err.sum()}", z_lim=z_lim, vmin=vmin, vmax=vmax)

cbar = fig.colorbar(im, ax=ax, shrink=0.8, pad=0.02)
cbar.set_label("Norm. Speed")

plt.tight_layout()
plt.savefig("./figures/single/fig2H_correct_vs_error_maps.svg")
plt.show()
#+end_src

#+RESULTS:
[[file:./figures/singleFlow/figure_41.png]]

---

**** 7. One combined draft figure for 2E–H

#+begin_src jupyter-python
fig = plt.figure(figsize=(12, 10))
gs = fig.add_gridspec(3, 2, height_ratios=[1.3, 0.8, 0.9])

# 2E
axE1 = fig.add_subplot(gs[0, 0])
axE2 = fig.add_subplot(gs[0, 1], sharex=axE1, sharey=axE1)

im, _ = plot_overlap_map_panel(axE1, xN, yN, trajs=trajN, title="Naive", z_lim=z_lim, vmin=vmin, vmax=vmax)
plot_overlap_map_panel(axE2, xE, yE, trajs=trajE, title="Expert", z_lim=z_lim, vmin=vmin, vmax=vmax)

# 2F
axF1 = fig.add_subplot(gs[1, 0])
axF2 = fig.add_subplot(gs[1, 1])

for mouse in wide_x.index:
    axF1.plot([0,1], [wide_x.loc[mouse, "Naive"], wide_x.loc[mouse, "Expert"]], color="0.7")
    axF1.scatter([0,1], [wide_x.loc[mouse, "Naive"], wide_x.loc[mouse, "Expert"]], s=40)

for mouse in wide_y.index:
    axF2.plot([0,1], [wide_y.loc[mouse, "Naive"], wide_y.loc[mouse, "Expert"]], color="0.7")
    axF2.scatter([0,1], [wide_y.loc[mouse, "Naive"], wide_y.loc[mouse, "Expert"]], s=40)

axF1.set_xticks([0,1]); axF1.set_xticklabels(["Naive", "Expert"])
axF2.set_xticks([0,1]); axF2.set_xticklabels(["Naive", "Expert"])
axF1.set_ylabel("Delay centroid\nA/B overlap")
axF2.set_ylabel("Delay centroid\nChoice overlap")
axF1.axhline(0, ls="--", color="k", alpha=0.5)
axF2.axhline(0, ls="--", color="k", alpha=0.5)

# 2G
axG = fig.add_subplot(gs[2, 0])
r, p = pearsonr(df_corr["d_choice_geom"], df_corr["d_gng"])
sns.regplot(
    data=df_corr, x="d_choice_geom", y="d_gng", ax=axG,
    scatter_kws=dict(s=70, edgecolor="black", linewidth=0.3),
    line_kws=dict(color="k", ls="--")
)
axG.axhline(0, ls="--", color="0.5")
axG.axvline(0, ls="--", color="0.5")
axG.set_xlabel(r"$\Delta$ choice overlap")
axG.set_ylabel(r"$\Delta$ GNG performance")
axG.set_title(f"r = {r:.2f}, p = {p:.3f}")

# 2H
axH = fig.add_subplot(gs[2, 1])
axH.plot(traj_corr["x_mean"], traj_corr["y_mean"], color="#1f77b4", lw=3, label="Correct")
axH.plot(traj_err["x_mean"],  traj_err["y_mean"],  color="#d62728", lw=3, label="Error")
axH.axhline(0, ls="--", color="k", alpha=0.5)
axH.axvline(0, ls="--", color="k", alpha=0.5)
axH.set_xlabel("A/B Overlap")
axH.set_ylabel("Choice Overlap")
axH.set_title("Unpaired DPA")
axH.legend(frameon=False)
axH.set_aspect("equal")

cax = fig.add_axes([0.92, 0.72, 0.015, 0.16])
cb = fig.colorbar(im, cax=cax)
cb.set_label("Norm. Speed")

plt.tight_layout(rect=[0, 0, 0.9, 1])
plt.savefig("./figures/single/figure2_EFGH_draft.svg")
plt.show()
#+end_src

---

*/ What probably changed relative to the failed version

These new blocks should behave much better because they:
- use /DPA only/ for the core geometry
- use /no extra scaling/
- use your real task labels: =DPA, DualGo, DualNoGo=
- use =odr_perf= for GNG-like behavior in panel G
- use =odor_pair in [1,3]= for unpaired DPA in panel H
- use condition-averaged trajectories overlaid on top of field maps

---

*/ If 2E still looks wrong

The next thing I would do is switch from pooled single-trial field estimation to /mouse-averaged trajectories/, i.e.:
- average per mouse, per sample
- then build the field from those trajectories only

That often gives cleaner manuscript-quality maps.

If you want, I can give you that version next. It will probably look even closer to your current draft panels.
...
def cv_pca(
    ...
    if clean_mask_fn is None:
        m_clean = (y.laser == 0) & (y.learning=='Expert') & (y.performance == 1) & ((y.tasks=='DPA') | (y.odr_perf==1))
    else:
...
#+RESULTS:
#+begin_example
JawsM01: 100% 90/90 [00:00<00:00, 201.57it/s]
JawsM06: 100% 219/219 [00:02<00:00, 82.94it/s]
JawsM12: 100% 137/137 [00:14<00:00,  9.31it/s]
JawsM15: 100% 208/208 [00:17<00:00, 12.14it/s]
JawsM18: 100% 278/278 [00:10<00:00, 27.63it/s]
ChRM04: 100% 240/240 [00:11<00:00, 21.46it/s]
ChRM23: 100% 131/131 [00:01<00:00, 126.57it/s]
ACCM03: 100% 318/318 [00:11<00:00, 27.75it/s]
ACCM04: 100% 269/269 [00:02<00:00, 122.16it/s]
#+end_example
...
#+RESULTS:
: (9216, 84, 10) (9216, 18) (10, 184) (219, 10)
...
: Cell In[22], line 1
: ----> 1 pkl_save(X_single, 'single_traj_' + dum, path="../data/pca/")
...
idx_correct =  (y_single.learning=='Naive') & (y_single.performance==1)
idx_mouse = True
if i_mouse !=-1:
...
#+RESULTS:
[[file:./figures/singleFlow/figure_65.png]]
...
#+RESULTS:
[[file:./figures/singlePCA/figure_59.png]]
...
#+RESULTS:
[[file:./figures/singlePCA/figure_71.png]]
