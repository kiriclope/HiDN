...
#+begin_src ipython
  ...
  for mouse in mice:
    ...
    pkl_save(coefs_save, '%s_coefs_%.2f_l1_ratio' % (options['mouse'], options['l1_ratio']), path="../data/%s/" % options['mouse'])
    #+end_src
...
[[file:./.ob-jupyter/78a6293c742b5066f4ded69aa5299a03da6f666c.png]]
:END:
...
[[file:./.ob-jupyter/d0c0e96f3c2c205fce153a763bc7f00cbc6e6822.png]]
:END:
...
#+RESULTS:
[[file:./.ob-jupyter/7114baa2c620ca7d24e7d67bc6939fa960c92daf.png]]
...
#+begin_src ipython
  ...
  for mouse in mice:
      print(mouse)
      try:
          coefs = pkl_load('%s_coefs_%.2f_l1_ratio.pkl' % (mouse, l1_ratio), path="../data/%s/" % mouse)
          print(coefs.shape)
...
#+RESULTS:
[[file:./.ob-jupyter/3a8bf55db26ab51d5329d71a25ae9c7543a07177.png]]
