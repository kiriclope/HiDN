#+STARTUP: fold
#+PROPERTY: header-args:ipython :results both :exports both :async yes :session dpca :kernel dual :output-dir ./figures/dpca :file (lc/org-babel-tangle-figure-filename)
...
#+RESULTS:
: b3edf6b3-b37d-4628-858d-80ee59e3fc5c
...
def _ridge_fit(Y, X, alpha):
    XXt = X @ X.T
    XXt.flat[:: XXt.shape[0] + 1] += float(alpha)
    rhs = Y @ X.T
    return np.linalg.solve(XXt.T, rhs.T).T


def _top_components_from_decoder(V, n_components):
    N = V.shape[0]
    k = int(n_components)
    if k <= 0:
...
class RegularizedDPCA(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y):
        ...
        cond_idx = _flat_from_coords(fidxs, dims)
        ...
        Xmat = Xc.transpose(1, 0, 2).reshape(N, -1)
                ...
                if r > 1:
                    for rr in range(1, r):
                        for subsub in combinations(subset, rr):
                            # effect(subsub) is currently flattened; reshape to its grid
                            dims_ss = [dims[i] for i in subsub]
                            Esub_grid = effects[subsub].reshape(*dims_ss, N, T)

                            # expand into subset grid by inserting singleton dims in missing axes
                            reshape_sizes = []
                            for i in subset:
                                reshape_sizes.append(dims[i] if i in subsub else 1)
                            Esub_in_subset = Esub_grid.reshape(*reshape_sizes, N, T)  # broadcasts
                            Egrid -= Esub_in_subset

                effects[subset] = Egrid.reshape(-1, N, T)

        # fit each marginal
        self.components_, self.decoders_ = {}, {}
        ...
        for m in self.marginalizations_:
            ...
            Ytrial = Econd[combo_id]                             # (B,N,T)
            Ymat = Ytrial.transpose(1, 0, 2).reshape(N, -1)

            V = _ridge_fit(Ymat, Xmat, self.alpha_)
            W = _top_components_from_decoder(V, self.n_components)
            self.decoders_[m] = V
    ...
    def transform(self, X, marginalization):
        ...
        return np.einsum("bnt,kn->bkt", Xc, W)

    def reconstruct_marginal(self, X, marginalization):
        check_is_fitted(self, "decoders_")
        X = np.asarray(X)
        if X.ndim != 3:
            raise ValueError("X must be (n_trials,n_neurons,n_time)")
        if X.shape[1] != self.n_neurons_ or X.shape[2] != self.n_time_:
        ...
        V = self.decoders_[m]
        Xc = X - self.grand_mean_[None, :, :]
        Xhat = np.einsum("nm,bmt->bnt", V, Xc)
    ...
    def _compute_evr_dict(self, X):
        """
        EVR per marginalization: Var(V_m Xc) / Var(Xc)
        Returns dict: key marginalization tuple -> float
        """
        X = np.asarray(X)
        Xc = X - self.grand_mean_[None, :, :]
        den = np.var(Xc) + 1e-12
        ...
        evr = {}
        for m, V in self.decoders_.items():
            Xhat = np.einsum("nm,bmt->bnt", V, Xc)
...
def _flatten_factors_to_cond(labels, factors):
    """
    labels: dict factor -> (n_trials,)
    returns:
      cond_idx: (n_trials,) flattened factorial condition index
      levels: dict factor -> sorted unique values
      all_keys: list of factors
      dims: list of n_levels per factor in same order
    """
    n_trials = len(next(iter(labels.values())))
    all_keys = list(factors)
    levels = {k: _unique_sorted(labels[k]) for k in all_keys}
    level_to_idx = {k: {val: i for i, val in enumerate(levels[k])} for k in all_keys}
    dims = [len(levels[k]) for k in all_keys]

    def flat_index(i):
        idxs = [level_to_idx[k][labels[k][i]] for k in all_keys]
        flat = 0
        mult = 1
        for d, ii in zip(reversed(dims), reversed(idxs)):
            flat += ii * mult
            mult *= d
        return flat

    cond_idx = np.fromiter((flat_index(i) for i in range(n_trials)), dtype=int, count=n_trials)
    return cond_idx, levels, all_keys, dims
...
def _compute_condition_means(X, cond_idx, n_cond):
    ...
    returns:
      means: (n_cond,n_neurons,n_time) with NaNs for empty conditions
      counts: (n_cond,)
      valid: (n_cond,) bool
    """
    n_trials, n_neurons, n_time = X.shape
    means = np.zeros((n_cond, n_neurons, n_time), float)
    counts = np.zeros((n_cond,), int)
    for i in range(n_trials):
        c = int(cond_idx[i])
        means[c] += X[i]
        counts[c] += 1
    valid = counts > 0
    means[valid] /= counts[valid, None, None]
    means[~valid] = np.nan
...
def _ridge_fit(Y, X, alpha):
    """
    Solve V in: Y ≈ V X with ridge penalty alpha||V||^2
    Y: (n_targets, n_samples)
    X: (n_features, n_samples)
    return V: (n_targets, n_features)
    """
    alpha = float(alpha)
    Xt = X.T
    XXt = X @ Xt
    reg = XXt + alpha * np.eye(XXt.shape[0])
    return (Y @ Xt) @ np.linalg.inv(reg)


def _top_components_from_decoder(V, n_components):
    """
    V: (p,p) decoder, generally NOT symmetric.
    Return components W: (k,p) using left singular vectors of V.
    """
    U, S, Vt = np.linalg.svd(V, full_matrices=False)
    k = min(int(n_components), U.shape[1])
    return U[:, :k].T


def _shrinkage_to_ridge_alpha(shrinkage, cov):
    """
    Convert shrinkage-to-identity level s to ridge alpha:
      C_shrunk = (1-s) C + s * mu I, mu=tr(C)/p
      alpha = (s/(1-s)) * mu
    """
    p = cov.shape[0]
    mu = np.trace(cov) / max(1, p)
...
class RegularizedDPCA(BaseEstimator, TransformerMixin):
    """
    Regularized dPCA using ridge regularization level chosen by
    Ledoit-Wolf ('lw') or OAS ('oas') covariance shrinkage.

    Parameters
    ----------
    factors : list[str]
        Factor names; y must be a dict with these keys, each (n_trials,).
    n_components : int
        Number of demixed components per marginalization.
    alpha : {'lw','oas'}
        Shrinkage estimator used to set ridge alpha.
    include_interactions : bool
        Include interaction marginalizations.
    include_ci : bool
        Include condition-independent ("time-only") marginalization ('ci',).
    center : bool
        If True, subtract time-varying grand mean (neurons x time) computed from
        condition means.
    fit_on_trials : bool
        If True, fit decoders using single-trial data with per-trial marginalized
        targets. If False, fit using condition means.

    Notes on CI
    -----------
    If center=True subtracts a full time-varying grand mean, the CI marginal in
    centered space often becomes ~0. Consider center=False and doing your own
    baseline subtraction if you want a meaningful CI.
    """
    ...
    def __init__(
        ...
        self.include_interactions = bool(include_interactions)
        self.include_ci = bool(include_ci)
        self.center = bool(center)
        self.fit_on_trials = bool(fit_on_trials)

    def _make_marginalizations(self):
        from itertools import combinations
        keys = self.factors
        marginals = []
        if self.include_ci:
            marginals.append(("ci",))
        marginals += [(k,) for k in keys]
        if self.include_interactions:
            for r in range(2, len(keys) + 1):
                for comb in combinations(keys, r):
                    marginals.append(tuple(comb))
        return marginals

    def _compute_effects(self, means_valid, cond_coords, all_keys):
        """
        means_valid: (n_valid,n_neurons,n_time) centered by some grand mean
        Returns effect(spec_keys) -> dict combo -> (n_neurons,n_time)
        """
        overall = np.nanmean(means_valid, axis=0)
        effect_cache = {}

        def avg_over(mask):
            return np.nanmean(means_valid[mask], axis=0)

        def effect(spec_keys):
            spec_keys = tuple(spec_keys)
            if spec_keys in effect_cache:
                return effect_cache[spec_keys]

            key_inds = [all_keys.index(k) for k in spec_keys]
            combos = np.unique(cond_coords[:, key_inds], axis=0)

            eff = {}
            for combo in combos:
                mask = np.ones((cond_coords.shape[0],), dtype=bool)
                for ki, val in zip(key_inds, combo):
                    mask &= (cond_coords[:, ki] == val)
                ...
                if len(spec_keys) > 1:
                    from itertools import combinations
                    for r in range(1, len(spec_keys)):
                        for sub_keys in combinations(spec_keys, r):
                            sub = effect(sub_keys)
                            sub_inds = [spec_keys.index(k) for k in sub_keys]
                            sub_combo = tuple(combo[i] for i in sub_inds)
                            e = e - sub[sub_combo]

                eff[tuple(combo)] = e

            effect_cache[spec_keys] = eff
            return eff

        return effect

    def _build_cond_struct(self, X, y):
        X = np.asarray(X)
        n_trials, n_neurons, n_time = X.shape

        labels = {k: _as_1d_labels(y[k]) for k in self.factors}
        cond_idx, levels, all_keys, dims = _flatten_factors_to_cond(labels, self.factors)
        n_cond = int(np.prod(dims))

        means, counts, valid = _compute_condition_means(X, cond_idx, n_cond)
        valid_ids = np.where(valid)[0]
        cond_coords = np.array([_unravel(cid, dims) for cid in valid_ids], int)

        means_valid_raw = means[valid]  # (n_valid,n_neurons,n_time) raw
        if means_valid_raw.size == 0:
            raise ValueError("No valid conditions (every condition count=0).")

        grand = np.nanmean(means_valid_raw, axis=0) if self.center else np.zeros((n_neurons, n_time))
        means_valid = means_valid_raw - grand[None, :, :]
        ...
        return dict(
            ...
            means_valid_raw=means_valid_raw,
            means_valid=means_valid,
            grand=grand,
        )

    def _build_trial_matrix(self, X, grand):
        Xc = X - grand[None, :, :]
        return Xc.transpose(1, 0, 2).reshape(X.shape[1], -1)

    def _build_condmean_matrix(self, means_valid):
        return means_valid.transpose(1, 0, 2).reshape(means_valid.shape[1], -1)

    def _ci_target_trial(self, *, means_valid_centered, n_trials):
        """Condition-independent time course in the centered space."""
        ci = np.nanmean(means_valid_centered, axis=0)  # (neurons,time)
        return np.broadcast_to(ci[None, :, :], (n_trials, ci.shape[0], ci.shape[1])).copy()

    def _ci_target_cond(self, *, means_valid_centered):
        """Repeat CI(t) for each condition row."""
        ci = np.nanmean(means_valid_centered, axis=0)  # (neurons,time)
        n_valid = means_valid_centered.shape[0]
        return np.broadcast_to(ci[None, :, :], (n_valid, ci.shape[0], ci.shape[1])).copy()

    def _trial_targets_for_marginal(self, *, all_keys, effect, m, cond_idx, valid, dims):
        """
        Per-trial target tensor Ytrial: (n_trials,n_neurons,n_time)
        Each trial is assigned the marginalized condition-mean effect for its condition.
        """
        if m == ("ci",):
            raise ValueError("CI targets should be built directly (_ci_target_*), not via _trial_targets_for_marginal.")

        n_trials = cond_idx.shape[0]
        eff = effect(m)
        if len(eff) == 0:
            raise ValueError(f"No effects available for marginal {m} (likely missing conditions).")

        first = next(iter(eff.values()))
        n_neurons, n_time = first.shape

        key_inds = [all_keys.index(k) for k in m]

        Ytrial = np.zeros((n_trials, n_neurons, n_time), float)
        for tr in range(n_trials):
            cid = int(cond_idx[tr])
            if not bool(valid[cid]):
                continue
            coords = _unravel(cid, dims)
            combo = tuple(np.array(coords, int)[key_inds])
            if combo in eff:
                Ytrial[tr] = eff[combo]
        return Ytrial

    def _estimate_alpha_from_shrinkage(self, Xmat, method):
        # Xmat: (neurons, samples); shrinkage on samples x features
        Z = Xmat.T
        Z = Z - Z.mean(axis=0, keepdims=True)
        if method == "lw":
            ...
            raise ValueError("method must be 'lw' or 'oas'")
        cov = est.covariance_
        shrink = float(est.shrinkage_)
        alpha = _shrinkage_to_ridge_alpha(shrink, cov)
        return alpha, shrink

    def _fit_with_alpha(self, X, y, alpha_value):
        X = np.asarray(X)
        n_trials, n_neurons, n_time = X.shape

        S = self._build_cond_struct(X, y)
        cond_idx = S["cond_idx"]
        levels = S["levels"]
        all_keys = S["all_keys"]
        dims = S["dims"]
        valid = S["valid"]
        cond_coords = S["cond_coords"]
        means_valid = S["means_valid"]
        grand = S["grand"]

        effect = self._compute_effects(means_valid, cond_coords, all_keys)
        marginals = self._make_marginalizations()

        if self.fit_on_trials:
            Xmat = self._build_trial_matrix(X, grand)
        else:
            Xmat = self._build_condmean_matrix(means_valid)
        ...
        for m in marginals:
            if m == ("ci",):
                if self.fit_on_trials:
                    Ytrial = self._ci_target_trial(means_valid_centered=means_valid, n_trials=n_trials)
                    Ymat = Ytrial.transpose(1, 0, 2).reshape(n_neurons, -1)
                else:
                    Ycond = self._ci_target_cond(means_valid_centered=means_valid)
                    Ymat = self._build_condmean_matrix(Ycond)
            else:
                if self.fit_on_trials:
                    Ytrial = self._trial_targets_for_marginal(
                        all_keys=all_keys,
                        effect=effect,
                        m=m,
                        cond_idx=cond_idx,
                    ...
                    effm = effect(m)
                    key_inds = [all_keys.index(k) for k in m]
                    Y = np.zeros_like(means_valid)
                    for i in range(means_valid.shape[0]):
                        combo = tuple(cond_coords[i, key_inds])
                        Y[i] = effm[combo]
                    Ymat = self._build_condmean_matrix(Y)

            V = _ridge_fit(Ymat, Xmat, alpha=alpha_value)          # (neurons,neurons)
            W = _top_components_from_decoder(V, self.n_components) # (k,neurons)
            components_[m] = W
            decoders_[m] = V
    ...
    def fit(self, X, y):
        ...
        # build centered structure to get grand mean & matrices
        S = self._build_cond_struct(X, y)
        if self.fit_on_trials:
            Xmat = self._build_trial_matrix(X, S["grand"])
        else:
            Xmat = self._build_condmean_matrix(S["means_valid"])

        self.alpha_, self.shrinkage_ = self._estimate_alpha_from_shrinkage(Xmat, self.alpha)
        fitted = self._fit_with_alpha(X, y, self.alpha_)

        for k, v in fitted.items():
            setattr(self, k, v)

        self.explained_variance_ratio_ = self._compute_evr_dict(X, per_time=False)
        return self

    def _compute_evr_dict(self, X, per_time=False):
        """Compute EVR dict over self.marginalizations_ using fitted decoders."""
        check_is_fitted(self, "decoders_")
        X = np.asarray(X)
        Xc = X - self.grand_mean_[None, :, :]
        ...
        for m in self.marginalizations_:
            ...
            Xhat = np.einsum("nm,bmt->bnt", V, Xc)
            if per_time:
                num = np.nanvar(Xhat, axis=(0, 1))
                den = np.nanvar(Xc, axis=(0, 1)) + 1e-12
                out[m] = num / den
            else:
                num = np.nanvar(Xhat)
                den = np.nanvar(Xc) + 1e-12
                out[m] = float(num / den)
        return out

    def transform(self, X, marginalization=("sample",)):
        check_is_fitted(self, "components_")
        X = np.asarray(X)
        if X.ndim != 3:
            raise ValueError("X must be (n_trials,n_neurons,n_time)")
    ...
    def reconstruct_marginal(self, X, marginalization=("sample",)):
        check_is_fitted(self, "decoders_")
        X = np.asarray(X)
        if X.ndim != 3:
            raise ValueError("X must be (n_trials,n_neurons,n_time)")
    ...
    def transform_demixed(self, X, marginalization=("sample",)):
        Xm = self.reconstruct_marginal(X, marginalization=marginalization)
        return self.transform(Xm, marginalization=marginalization)
#+end_src
...
def cross_val_avg_dpca(X, y, model, folds, factors):
...
#+RESULTS:
#+begin_example
JawsM01: 100% 5/5 [00:12<00:00,  2.52s/it]
JawsM06: 100% 5/5 [00:34<00:00,  6.95s/it]
JawsM12: 100% 5/5 [01:34<00:00, 18.93s/it]
JawsM15: 100% 5/5 [06:32<00:00, 78.46s/it]
JawsM18: 100% 5/5 [03:26<00:00, 41.31s/it]
ChRM04: 100% 5/5 [06:37<00:00, 79.58s/it]
ChRM23: 100% 5/5 [00:29<00:00,  5.81s/it]
ACCM03: 100% 5/5 [02:21<00:00, 28.36s/it]
ACCM04: 100% 5/5 [00:12<00:00,  2.41s/it]
#+end_example
...
#+begin_src ipython
laser_mice = ['JawsM06', 'JawsM12', 'JawsM18', 'ChRM04']
# laser_mice = ['JawsM01', 'JawsM06', 'JawsM12', 'JawsM15','JawsM18']
...
#+RESULTS:
[[file:./figures/dpca/figure_179.png]]
...
#+RESULTS:
: (8, 3) (8, 3)
: [ 0.00694444  0.03472222  0.00694444 -0.04166667  0.01041667  0.09375
...
#+RESULTS:
: (8, 3) (8, 3)
: [-0.05208333 -0.02083333 -0.02083333  0.          0.          0.015625
...
#+RESULTS:
:      mouse  sample_odor  delta_dpa  delta_odr  delta_pc1  delta_pc2
: 2  JawsM06          0.0   0.006944  -0.020833  -0.153215  -0.485115
: 3  JawsM06          1.0  -0.041667   0.000000  -0.045689  -0.389152
: 4  JawsM12          0.0   0.010417   0.000000  -0.261653   0.646642
: 5  JawsM12          1.0   0.093750   0.015625  -0.048802   0.138196
: 6  JawsM18          0.0   0.006944  -0.020833   0.237359   0.067938
...
#+RESULTS:
[[file:./figures/dpca/figure_184.png]]
