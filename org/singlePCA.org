...
#+RESULTS:
: b4415bb2-d130-460d-bc40-4fc35b1d74bc
...
#+RESULTS:
: saving to ../data/pca/X_all_nan_center.pkl
: saving to ../data/pca/y_all_nan_center.pkl
...
def cv_pca(
    ...
    if clean_mask_fn is None:
        m_clean = (y.laser == 0) & (y.learning=='Expert') & (y.performance == 1) & (y.tasks=='DPA') # & ((y.tasks=='DPA') | (y.odr_perf==1))
    else:
    ...
    X_scaled = Xc
    if scale is not None:
        scaler = StandardScaler(if_scale=1)
        scaler.fit(Xc)
    ...
    # -----------------------------
    Xc_epoch = X_scaled if epoch is None else X_scaled[..., epoch]
    ...
    if m_pert.mean() != 0:
        ...
        return Z_all, y_all, W_all, evr_folds
...
def cv_pca(
    ...
    show_pbar=True,
    clean_mask_fn=None,
    enforce_det_pos=False,
    center_on="trialtime",   # "trialtime" or "condavg"
    align_folds=True,
    strict_cv=False,         # if False, use all-clean reference for fold alignment
    return_fold_loadings=False,
):
    """
    Per-animal CV condition-average PCA.

    Main behavior:
    - Fit a canonical PCA on ALL clean trials (condition-averaged) for this animal.
    - Use that canonical PCA to project perturbed trials directly (NO extra within-animal
      alignment of perturbed data).
    - For clean trials, do CV:
        ,* fit PCA on TRAIN condition averages
        ,* project TEST trials
        ,* optionally align fold latent space to canonical clean reference
    - Return all trajectories in one common within-animal latent space.

    Parameters
    ----------
    X : array, shape (n_trials, n_neurons, n_time)
    y : DataFrame, aligned to trials
    pca : sklearn PCA-like estimator
    folds : CV splitter
    factors : str or list of str
        Columns defining condition combinations used for condition averages / anchors.
    epoch : None or slice/indexer
        Time window used for fitting/alignment. Projection is always returned on full time.
    scale : None or scaler object
        Must support fit/transform on X shaped (n_trials, n_neurons, n_time).
    group_col : str or None
    show_pbar : bool
    clean_mask_fn : callable or None
    enforce_det_pos : bool
    center_on : {"trialtime", "condavg"}
    align_folds : bool
        If True, align fold-specific test trajectories to canonical all-clean reference.
    strict_cv : bool
        If True, use first fold's train reference as the alignment reference instead of
        all-clean reference. Avoids leakage from test trials into alignment reference.
    return_fold_loadings : bool

    Returns
    -------
    Z_all : (n_return_trials, n_time, n_comp)
    y_all : DataFrame
    W_ref : (n_comp, n_neurons)
        Canonical all-clean loading matrix.
    evr_folds : object array of per-fold EVRs
    extras : dict
    """
    if isinstance(factors, str):
        factors = [factors]
    ...
    def maybe_scale_fit_transform(scaler, A):
        if scaler is None:
            return A, None
        sc = clone(scaler)
        return sc.fit_transform(A), sc

    def maybe_scale_transform(sc, A):
        if sc is None:
            return A
        return sc.transform(A)

    def compute_mean(X_epoch, X_avg_flat, how):
        if how == "trialtime":
            return np.nanmean(flat_trial_time(X_epoch), axis=0, keepdims=True)
        elif how == "condavg":
            return np.nanmean(X_avg_flat, axis=0, keepdims=True)
        else:
            raise ValueError("center_on must be 'trialtime' or 'condavg'")

    def fit_pca_on_train(X_train, y_train, levels):
        X_train_epoch = X_train if epoch is None else X_train[..., epoch]
        X_avg_epoch, combos = cv_avg_cond(X_train_epoch, y_train, factors, levels, drop_missing=True)
        X_avg_flat = flat_trial_time(X_avg_epoch)

        X_mean = compute_mean(X_train_epoch, X_avg_flat, center_on)
        X_fit = X_avg_flat - X_mean
        if np.isnan(X_fit).any():
            raise ValueError("NaNs detected in PCA fit matrix.")

        p = clone(pca)
        p.fit(X_fit)
        return p, X_mean, combos, X_train_epoch

    def project_trials(p, X_full, X_mean):
        Z = p.transform(flat_trial_time(X_full) - X_mean)
        return Z.reshape(X_full.shape[0], X_full.shape[-1], -1)

    def anchors_with_combos(Z_epoch, y_df, levels):
        combos_all = list(itertools.product(*[levels[f] for f in factors]))
        A_list, combos_present = [], []
        for combo in combos_all:
            idx = _combo_mask(y_df, factors, combo)
            if not idx.any():
                continue
            A_list.append(np.nanmean(Z_epoch[idx], axis=0))   # (n_time_epoch, n_comp)
            combos_present.append(combo)
        if len(A_list) == 0:
            return None, []
        A = np.stack(A_list, axis=0).reshape(-1, Z_epoch.shape[-1])
        return A, combos_present

    def intersect_anchor_rows(A1, combos1, A2, combos2, n_time_epoch):
        set2 = {c: i for i, c in enumerate(combos2)}
        rows1, rows2 = [], []
        for i, c in enumerate(combos1):
            if c in set2:
                j = set2[c]
                r1 = np.arange(i * n_time_epoch, (i + 1) * n_time_epoch)
                r2 = np.arange(j * n_time_epoch, (j + 1) * n_time_epoch)
                rows1.append(r1)
                rows2.append(r2)
        if len(rows1) == 0:
            return None, None
        rows1 = np.concatenate(rows1)
        rows2 = np.concatenate(rows2)
        return A1[rows1], A2[rows2]

    def solve_rotation(A_src, A_tgt):
        A0 = A_src - A_src.mean(axis=0, keepdims=True)
        B0 = A_tgt - A_tgt.mean(axis=0, keepdims=True)
    ...
    m_pert = ~m_clean
    Xp = X[m_pert] if np.any(m_pert) else None
    yp = y.loc[m_pert].reset_index(drop=True) if np.any(m_pert) else None

    # deterministic condition ordering from all clean data
    levels = get_levels(yc, factors)

    # -----------------------------
    # canonical all-clean PCA
    # -----------------------------
    Xc_all, sc_all = maybe_scale_fit_transform(scale, Xc)
    Xc_epoch_all = Xc_all if epoch is None else Xc_all[..., epoch]

    X_avg_epoch_all, combos_all = cv_avg_cond(Xc_epoch_all, yc, factors, levels, drop_missing=True)
    X_avg_flat_all = flat_trial_time(X_avg_epoch_all)
    X_mean_all = compute_mean(Xc_epoch_all, X_avg_flat_all, center_on)

    X_fit_all = X_avg_flat_all - X_mean_all
    if np.isnan(X_fit_all).any():
        raise ValueError("NaNs detected in all-clean PCA fit matrix.")

    pca_all = clone(pca)
    pca_all.fit(X_fit_all)

    W_ref = pca_all.components_
    evr_all = pca_all.explained_variance_ratio_

    # canonical anchor reference from all-clean projected clean trials
    Zc_epoch_all = project_trials(pca_all, Xc_epoch_all, X_mean_all)
    A_ref_all, combos_ref_all = anchors_with_combos(Zc_epoch_all, yc, levels)
    n_time_epoch_ref = Xc_epoch_all.shape[-1]
    ...
    strata = yc[factors].astype(str).agg("_".join, axis=1).values
    ...
    Z_folds, y_folds = [], []
    W_folds_aligned, evr_folds = [], []

    A_ref_cv, combos_ref_cv, n_time_epoch_cv = None, None, None

    for fold, (train, test) in enumerate(it):
        X_train, y_train = Xc[train], yc.iloc[train].reset_index(drop=True)
        X_test, y_test = Xc[test], yc.iloc[test].reset_index(drop=True)

        X_train_s, sc_fold = maybe_scale_fit_transform(scale, X_train)
        X_test_s = maybe_scale_transform(sc_fold, X_test)

        pca_fold, X_mean_fold, combos_train, X_train_epoch = fit_pca_on_train(X_train_s, y_train, levels)
        X_test_full = X_test_s

        evr_folds.append(pca_fold.explained_variance_ratio_)

        Z_test = project_trials(pca_fold, X_test_full, X_mean_fold)
        W_fold = pca_fold.components_

        if align_folds:
            Z_train_epoch = project_trials(pca_fold, X_train_epoch, X_mean_fold)
            A_train, combos_train_present = anchors_with_combos(Z_train_epoch, y_train, levels)
            n_time_epoch = X_train_epoch.shape[-1]

            if A_train is None:
                raise ValueError(f"No anchor conditions present in fold {fold} train set.")

            if strict_cv:
                if A_ref_cv is None:
                    A_ref_cv = A_train
                    combos_ref_cv = combos_train_present
                    n_time_epoch_cv = n_time_epoch
                    R = np.eye(Z_test.shape[-1])
                else:
                    A_src, A_tgt = intersect_anchor_rows(
                        A_train, combos_train_present,
                        A_ref_cv, combos_ref_cv,
                        n_time_epoch_cv
                    )
                    if A_src is None:
                        raise ValueError(f"No shared anchor combos between fold {fold} and CV reference.")
                    R = solve_rotation(A_src, A_tgt)
            else:
                A_src, A_tgt = intersect_anchor_rows(
                    A_train, combos_train_present,
                    A_ref_all, combos_ref_all,
                    n_time_epoch_ref
                )
                if A_src is None:
                    raise ValueError(f"No shared anchor combos between fold {fold} and all-clean reference.")
                R = solve_rotation(A_src, A_tgt)
    ...
    # -----------------------------
    # project perturbed directly into canonical clean PCA space
    # NO extra within-animal rotation
    # -----------------------------
    if Xp is not None:
        Xp_all = maybe_scale_transform(sc_all, Xp)
        Zp = project_trials(pca_all, Xp_all, X_mean_all)
    ...
    extras = {
        ...
        "evr_all": evr_all,
        "A_ref_all": A_ref_all,
        "combos_ref_all": combos_ref_all,
    }

    if return_fold_loadings:
        extras["W_folds_aligned"] = W_folds_aligned
        extras["W_fold_mean"] = np.mean(np.stack(W_folds_aligned, axis=0), axis=0)

    return Z_all, y_all, W_ref, evr_folds, extras
#+end_src
...
for mouse in options['mice']:
    ...
    Z_mouse, y_mouse, W_mouse, evr_mouse, _ = cv_pca(X_pca, y_pca, pca, folds, factors, epoch, center_on="trialtime")
...
#+RESULTS:
#+begin_example
JawsM01: 100% 31/31 [00:00<00:00, 272.14it/s]
JawsM06: 100% 84/84 [00:00<00:00, 180.39it/s]
JawsM12: 100% 50/50 [00:00<00:00, 147.42it/s]
JawsM15: 100% 89/89 [00:14<00:00,  6.11it/s]
JawsM18: 100% 95/95 [00:01<00:00, 77.49it/s]
ChRM04: 100% 88/88 [00:13<00:00,  6.30it/s]
ChRM23: 100% 48/48 [00:00<00:00, 223.79it/s]
ACCM03: 100% 122/122 [00:01<00:00, 72.21it/s]
ACCM04: 100% 93/93 [00:00<00:00, 123.36it/s]
#+end_example

#+begin_src jupyter-python
def anchors_with_combos(Z, y, factors, levels):
    if isinstance(factors, str):
        factors = [factors]
    ...
    for combo in combos_all:
        ...
        if not idx.any():
            continue
        A_list.append(np.nanmean(Z[idx], axis=0))   # (n_time, n_comp)
        combos_present.append(combo)

    if len(A_list) == 0:
        return None, []

    A = np.stack(A_list, axis=0).reshape(-1, Z.shape[-1])
    return A, combos_present
#+end_src
...
#+begin_src jupyter-python
print(X_aligned_all.shape, y_aligned_all.shape, w_mice[0].shape, evr_mice[1].shape)
#+end_src

#+RESULTS:
: (9216, 84, 10) (9216, 18) (10, 184) (84, 10)
...
print(w_stack.shape, evr_single.shape)
#+end_src
...
#+begin_src jupyter-python
# E = pc_mouse_energy(w_stack, mouse_slices)  # DataFrame: PC x mouse
...
#+RESULTS:
[[file:./figures/singlePCA/figure_36.png]]
...
#+RESULTS:
: saving to ../data/pca//single_traj_TESTDPA.pkl
: saving to ../data/pca//single_labels_TESTDPA.pkl
: saving to ../data/pca//single_weights_TESTDPA.pkl
: saving to ../data/pca//single_evr_TESTDPA.pkl
...
#+RESULTS:
: loading from ../data/pca//single_traj_TEST.pkl
: loading from ../data/pca//single_labels_TEST.pkl
: loading from ../data/pca//single_weights_TEST.pkl
: loading from ../data/pca//single_evr_TEST.pkl
...
idx_correct =  (y_single.learning=='Naive') & (y_single.performance==1) & (y_single.tasks=='DPA')
idx_mouse = True
if i_mouse !=-1:
...
#+RESULTS:
: (9216, 10, 84) (9216, 18)
: Using bins_state: [18 19 20] ... [51 52 53]
...
#+RESULTS:
[[file:./figures/singlePCA/figure_50.png]]
...
|---+---------+----------+------------+------------+----------+----------+----------|
| 0 | JawsM01 | Naive    | -7.368778  | 1.257271   | 7.368778 | 2.071561 | 75       |
| 1 | JawsM01 | Expert   | -7.608942  | -0.410214  | 7.608942 | 1.916405 | 31       |
| 2 | JawsM06 | Naive    | -3.485188  | -0.056764  | 3.518080 | 1.804923 | 60       |
| 3 | JawsM06 | Expert   | -3.006540  | -0.231667  | 3.182809 | 2.155885 | 84       |
| 4 | JawsM12 | Naive    | -4.187242  | 0.560836   | 4.386398 | 1.690901 | 62       |
:END:
...
#+RESULTS:
[[file:./figures/singlePCA/figure_52.png]]
...
#+RESULTS:
[[file:./figures/singlePCA/figure_53.png]]
...
|---+---------+------------+-----------+------------+-----------+-----------+------------+-----------+------------+-----------+-----------+----------+-----------|
| 0 | JawsM01 | -7.368778  | 1.257271  | -7.608942  | -0.410214 | 0.781250  | 0.968750   | 0.947917  | 0.984375   | -0.240164 | -1.667484 | 0.187500 | 0.036458  |
| 1 | JawsM06 | -3.485188  | -0.056764 | -3.006540  | -0.231667 | 0.625000  | 0.875000   | 0.875000  | 0.776042   | 0.478647  | -0.174903 | 0.250000 | -0.098958 |
| 2 | JawsM12 | -4.187242  | 0.560836  | -2.353736  | 0.169049  | 0.645833  | 0.781250   | 0.791667  | 0.945312   | 1.833506  | -0.391787 | 0.135417 | 0.153646  |
| 3 | JawsM15 | -18.452473 | -1.479109 | -17.858196 | -1.116282 | 0.729167  | 0.927083   | 0.744792  | 0.718750   | 0.594277  | 0.362827  | 0.197917 | -0.026042 |
| 4 | JawsM18 | -29.304064 | 0.866758  | -26.570036 | 0.614078  | 0.833333  | 0.989583   | 0.848958  | 0.968750   | 2.734027  | -0.252680 | 0.156250 | 0.119792  |
| 5 | ChRM04  | -49.765451 | 4.322640  | -46.169255 | -0.390953 | 0.833333  | 0.916667   | 0.890625  | 0.859375   | 3.596197  | -4.713593 | 0.083333 | -0.031250 |
| 6 | ChRM23  | -14.715155 | 0.860540  | -12.511259 | 1.722979  | 0.656250  | 0.750000   | 0.697917  | 0.820312   | 2.203896  | 0.862440  | 0.093750 | 0.122396  |
| 7 | ACCM03  | -25.010011 | 3.996422  | -21.499245 | 0.491592  | 0.692708  | 0.953125   | 0.692708  | 0.871094   | 3.510766  | -3.504830 | 0.260417 | 0.178385  |
| 8 | ACCM04  | -8.716558  | 1.223649  | -8.433033  | 1.114868  | 0.562500  | 0.726562   | 0.609375  | 0.910156   | 0.283524  | -0.108782 | 0.164062 | 0.300781  |
:END:
...
#+RESULTS:
[[file:./figures/singlePCA/figure_55.png]]
...
Cell In[155], line 32
     29 df_err = pd.DataFrame(vals)
     31 if len(df_err):
---> 32     sns.pointplot(data=df_err, x='type', y='late_choice', dodge=0.2, errorbar='se', ax=ax[1], color='k')
     33     sns.stripplot(data=df_err, x='type', y='late_choice', hue='mouse', dodge=False, ax=ax[1], size=6)
     34     ax[1].legend([], [], frameon=False)

File ~/mambaforge/envs/dual/lib/python3.14/site-packages/seaborn/categorical.py:2516, in pointplot(data, x, y, hue, order, hue_order, estimator, errorbar, n_boot, seed, units, weights, color, palette, hue_norm, markers, linestyles, dodge, log_scale, native_scale, orient, capsize, formatter, legend, err_kws, ci, errwidth, join, scale, ax, **kwargs)
   2513 p._point_kwargs_backcompat(scale, join, kwargs)
   2514 err_kws, capsize = p._err_kws_backcompat(err_kws, None, errwidth, capsize)
-> 2516 p.plot_points(
   2517     aggregator=aggregator,
   2518     markers=markers,
   2519     linestyles=linestyles,
   2520     dodge=dodge,
   2521     color=color,
   2522     capsize=capsize,
   2523     err_kws=err_kws,
   2524     plot_kws=kwargs,
   2525 )
   2527 p._add_axis_labels(ax)
   2528 p._adjust_cat_axis(ax, axis=p.orient)

File ~/mambaforge/envs/dual/lib/python3.14/site-packages/seaborn/categorical.py:1217, in _CategoricalPlotter.plot_points(self, aggregator, markers, linestyles, dodge, color, capsize, err_kws, plot_kws)
...
#+end_example
[[file:./figures/singlePCA/figure_57.png]]
:END:
...
#+RESULTS:
[[file:./figures/singlePCA/figure_98.png]]
...
#+RESULTS:
[[file:./figures/singlePCA/figure_99.png]]
...
#+RESULTS:
[[file:./figures/singlePCA/figure_100.png]]
...
#+RESULTS:
[[file:./figures/singlePCA/figure_102.png]]
...
#+RESULTS:
[[file:./figures/singlePCA/figure_107.png]]
...
#+RESULTS:
[[file:./figures/singlePCA/figure_112.png]]
...
#+RESULTS:
[[file:./figures/singlePCA/figure_113.png]]
...
#+RESULTS:
[[file:./figures/singlePCA/figure_114.png]]
...
#+RESULTS:
[[file:./figures/singlePCA/figure_142.png]]
