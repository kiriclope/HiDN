...
#+begin_src jupyter-python
X_all, y_all, mouse_slices = build_padded_X(options, n_neurons_total=3319)
...
def scaler(X, std_=0):
    ...
    # mean_ = np.nanmean(X, axis=(0, -1))[:, np.newaxis]

    if std_:
        std = np.nanstd(X, axis=0)
        std_floor = np.percentile(std, 5)
        std_safe = np.maximum(std, std_floor)
        return (X - mean_) / std_safe, mean_, std_safe

    return (X - mean_), mean_, 1
...
def cv_pca(
    ...
    m_clean = (y.laser == 0) & (y.tasks=='DPA') # & (y.performance == 1) & ((y.tasks=='DPA') | (y.odr_perf==1))
    Xc = X[m_clean]
    yc = y.loc[m_clean].reset_index(drop=True)

    # global scaling based on clean data (applied to all splits)
    Xc_scaled, mean_global, std_global = scaler(Xc)
    ...
    for train_idx, test_idx in splitter:
        ...
        # per-fold scaling (optional). If you want strictly global scaling, remove next 2 lines.
        X_train, mean_fold, std_fold = scaler(X_train)
        X_test = (X_test - mean_fold) / std_fold
...
#+begin_src jupyter-python
X_cv, y_cv, w_cv, w_all, evr_cv = cv_pca(X_pca, y_pca, pca, folds, factors, epoch, mouse_slices=mouse_slices, mouse_gain_mode=None)
#+end_src

#+RESULTS:
: cv_pca: 100% 5/5 [00:07<00:00,  1.56s/it]
...
#+RESULTS:
: (4032, 84, 15) (4032, 18) (15, 3319) (15, 3319) (5, 15)
...
#+RESULTS:
: saving to ../data/pca//meta_traj_TEST_Expert.pkl
: saving to ../data/pca//meta_labels_TEST_Expert.pkl
: saving to ../data/pca//meta_weights_TEST_Expert.pkl
: saving to ../data/pca//meta_evr_TEST_Expert.pkl
...
#+RESULTS:
: loading from ../data/pca//meta_traj_all_Expert.pkl
: loading from ../data/pca//meta_labels_all_Expert.pkl
: loading from ../data/pca//meta_weights_all_Expert.pkl
: loading from ../data/pca//meta_evr_all_Expert.pkl
...
idx_correct = 1 # (y_meta.performance==1) # & (y_meta.odr_perf==1)
idx_mouse = True
if i_mouse !=-1:
...
for i_mouse, mouse in enumerate(laser_mice):
    ...
    traj_ = []
    for i in range(2):
        mask = (y_idx.sample_odor==i) & (y_idx.tasks.isin(tasks))
...
: 4  JawsM01          0.0   0.000000   0.031250   0.022104   0.018956
: 5  JawsM01          1.0   0.000000  -0.031250   0.002658   0.053433
: 6  JawsM06          0.0   0.006944  -0.020833   0.002763  -0.001678
: 7  JawsM06          1.0  -0.041667   0.000000  -0.019076   0.009967
: 8  JawsM12          0.0   0.010417   0.000000  -0.096039   0.053336
