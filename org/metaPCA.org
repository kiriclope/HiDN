...
class StandardScaler:
    ...
    def fit(self, X):
        ...
        # p5 = np.nanpercentile(self.scale_, 5)
        # std_floor = 1.0 if (not np.isfinite(p5) or p5 < 1e-4) else p5
        # self.scale_ = np.maximum(self.scale_, std_floor)

        self.scale_ = np.where(self.scale_ < 1e-4, 1, self.scale_)
...
#+RESULTS:
: saving to ../data/pca/X_all_.pkl
: saving to ../data/pca/y_all_.pkl
...
def cv_pca(
    ...
    m_clean = (y.laser == 0) & (y.learning==learning) & (y.tasks=='DPA') # & (y.performance==1) # & ((y.tasks=='DPA') | (y.odr_perf==1))
    Xc = X[m_clean]
...
#+begin_src jupyter-python
X_cv, y_cv, w_cv, w_all, evr_cv = cv_pca(X_pca, y_pca, pca, folds, factors, epoch, mouse_slices=mouse_slices, mouse_gain_mode=None, scale=1, scale_test=1)
#+end_src

#+RESULTS:
: cv_pca: 100% 5/5 [03:19<00:00, 39.91s/it]
...
#+RESULTS:
: saving to ../data/pca//meta_traj_all.pkl
: saving to ../data/pca//meta_labels_all.pkl
: saving to ../data/pca//meta_weights_all.pkl
: saving to ../data/pca//meta_evr_all.pkl
...
#+RESULTS:
: loading from ../data/pca//meta_traj_all_scale.pkl
: loading from ../data/pca//meta_labels_all_scale.pkl
: loading from ../data/pca//meta_weights_all_scale.pkl
: loading from ../data/pca//meta_evr_all_scale.pkl
...
: 0  JawsM01          0.0   0.000000   0.031250  -0.036933   0.070300
: 1  JawsM01          0.0   0.041667   0.020833  -0.023818   0.045946
: 2  JawsM01          1.0   0.000000  -0.031250  -0.041590   0.060454
: 3  JawsM01          1.0  -0.006944   0.052083   0.001180   0.052342
: 4  JawsM06          0.0   0.006944  -0.020833  -0.004883  -0.003746
...
for i, delta_perf in enumerate(['delta_dpa', 'delta_odr']):
    ...
    ax[i].annotate(annotation, xy=(.65, 0.15), xycoords='axes fraction', fontsize=14,
                backgroundcolor='white', verticalalignment='bottom', horizontalalignment='left',
                bbox=dict(edgecolor=None, facecolor='white', boxstyle='round'))
