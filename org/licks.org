STARTUP: fold
#+PROPERTY: header-args:ipython :results both :exports both :async yes :session licks :kernel dual
...
def licks_raster(trial_licks, stimulus_onsets=None, ax=None, color='k'):
    ...
    ax.eventplot(trial_licks, colors=color, lineoffsets=1, linelengths=1, orientation='horizontal', rasterized=1)
    add_vlines2(ax=ax)
...
window_start = 7.0  # seconds after stimulus
window_end   = 7.5   # seconds after stimulus
window_length = window_end - window_start

window_start2 = 4.5  # seconds after stimulus
window_end2  = 5.5   # seconds after
window_length2 = window_end2 - window_start2
    ...
    for day in range(1, options['n_days']+1):
        ...
        try:
            ...
            for i, licks_in_trial in enumerate(trial_licks):
                ...
                rel_licks = licks_in_trial # - stimulus_onsets[i]
                # Count licks within [6.5, 7.0) s after onset
                n_licks = np.sum((rel_licks >= window_start2) & (rel_licks < window_end2))
                # Lick rate = count / window
                rate = n_licks * options['n_days'] / window_length2  # in Hz per day
                avg_lick_rates2.append(rate)

            avg_lick_rates2 = np.array(avg_lick_rates2)
...
:        'tasks', 'licks_LD', 'licks_MD', 'day', 'mouse'],
:       dtype='object')
...
Method:                          IRLS   Log-Likelihood:                -1965.7
Date:                Mon, 13 Apr 2026   Deviance:                       3931.5
Time:                        11:20:31   Pearson chi2:                 3.65e+03
No. Iterations:                     4   Pseudo R-squ. (CS):          0.0001570
Covariance Type:            nonrobust
==============================================================================
                 coef    std err          z      P>|z|      [0.025      0.975]
------------------------------------------------------------------------------
Intercept      1.2200      0.042     29.324      0.000       1.138       1.302
licks_LD      -0.0065      0.009     -0.763      0.445      -0.023       0.010
==============================================================================
#+end_example
...
formula = 'performance ~ licks_LD * day + (1 + day | mouse)'
...
: Cell In[14], line 9
:       5 df_ = df_[(df_.outcome==3) | (df_.outcome==4)]
:       7 formula = 'performance ~ licks_LD * day + (1 + day | mouse)'
: ----> 9 model = Lmer(formula=formula, data=df_, family='binomial')
:      10 results = model.fit()
:      11 random_effects = model.ranef
:
...
plt.xticks(np.arange(4),['Intercept', 'Late Delay Licks', 'Day', 'Late Delay Licks * Day'], rotation=45, fontsize=14)
# plt.yticks([-3, 0, -3])
plt.ylim([-3, 3])
...
: NameError                                 Traceback (most recent call last)
: Cell In[17], line 1
: ----> 1 plot_betas(results, random_effects, '', height=1.25*height)
:       2 plt.xticks(np.arange(4),['Intercept', 'Late Delay Licks', 'Day', 'Late Delay Licks * Day'], rotation=45, fontsize=14)
:       3 # plt.yticks([-3, 0, -3])
:
: NameError: name 'random_effects' is not defined
:END:
...
licks_raster(df_.licks, ax=ax[0], color=pal[0])

df_ = df_mouse[(df_mouse.tasks=='DualGo') & (df_mouse.laser!=-1)]
licks_raster(df_.licks, ax=ax[1], color=pal[1])

df_ = df_mouse[(df_mouse.tasks=='DualNoGo') & (df_mouse.laser!=-1)]
licks_raster(df_.licks, ax=ax[2], color=pal[2])
...
#+RESULTS:
[[file:./.ob-jupyter/7f36a071a18621335d034505bbc931287a42abf1.png]]
...
def licks_raster_all(trial_type, trial_licks, stimulus_onsets=None, ax=None, color='k'):
    ...
    for trial_idx, licks_in_trial in enumerate(trial_licks):
        if trial_type.iloc[trial_idx]=='DPA':
            ax.vlines(licks_in_trial, trial_idx , trial_idx + 10, color=pal[0], lw=1, rasterized=1)
        if trial_type.iloc[trial_idx]=='DualGo':
            ax.vlines(licks_in_trial, trial_idx , trial_idx + 10, color=pal[1], lw=1, rasterized=1)
        if trial_type.iloc[trial_idx]=='DualNoGo':
            ax.vlines(licks_in_trial, trial_idx , trial_idx + 10, color=pal[2], lw=1, rasterized=1)

    add_vlines2(ax=ax)
    ax.set_xlabel('Time from Sample Onset (s)')
    ax.set_ylabel('Trial #')
    ax.set_ylim(0, len(trial_licks))
    ax.set_yticks(np.linspace(0, len(trial_licks), 4))
...
print(df_.shape)
licks_raster_all(df_.tasks, df_.licks, ax=None)
plt.savefig('figures/licks/licks_raster_all.png', dpi=300)
#+end_src
...
#+RESULTS:
[[file:./.ob-jupyter/10deed4b0e6e50de034aea6108b8f8d843b7d6cc.png]]
...
#+RESULTS:
[[file:./.ob-jupyter/295f61b1a996a69c4286295d3dc29b649afd36e3.png]]
...
#+RESULTS:
[[file:./.ob-jupyter/5061664757c4312cd62e9e8df745843784671da8.png]]
...
#+RESULTS:
[[file:./.ob-jupyter/23e0dcc71a9795be9474ca291d51ffc531e63cbf.png]]
...
#+RESULTS:
[[file:./.ob-jupyter/56472014aa5c61c22ef5ec9563830a4443cbff96.png]]
...
: Index(['sample', 'test', 'outcome', 'pair', 'distractor', 'cue', 'odr_outcome',
:        'odr_pair', 'laser', 'licks', 'performance', 'odr_perf', 'paired',
:        'tasks', 'licks_LD', 'licks_MD', 'day', 'mouse', 'trial'],
:       dtype='object')
...
for i, task in enumerate(tasks_to_plot):
    ...
    # axes.set_ylim([0, 7])
    axes.set_yticks(np.linspace(1, 7, 4))
...
#+RESULTS:
[[file:./.ob-jupyter/709b731b6db9c1a7744ff41d3360f176b9a7b220.png]]
...
df_off = df_filtered[df_filtered.laser==0]
df_off = df_off[(df_off.outcome==3) | (df_off.outcome==4)]
sns.lineplot(data=df_off, x='day', y=df_off['licks_LD'] , ax=axes, estimator='mean', marker='o', markeredgecolor='none', label='Pre Test')

df_off = df_filtered[df_filtered.laser==0]
df_off = df_off[df_off.tasks=='DualNoGo']
sns.lineplot(data=df_off, x='day', y=df_off['licks_MD'] , ax=axes, estimator='mean', marker='o', markeredgecolor='none', label='Pre Cue')

axes.set_xlabel('Day')
axes.set_ylabel('Licks (Hz)')
axes.legend(fontsize=12, frameon=0)
# axes.set_ylim([0, 7])
# axes.set_yticks(np.linspace(1, 7, 4))
axes.set_xticks(np.arange(1, 7, 1))

plt.savefig('figures/bernstein/pre_licks_day_condition.svg', dpi=300)
...
#+RESULTS:
[[file:./.ob-jupyter/d0f2991ad1625c55205b681150e67a4c5e87dba2.png]]
...
#+RESULTS:
[[file:./.ob-jupyter/842343f57abc47521987c8b51bc3da9981a81c43.png]]
...
#+RESULTS:
[[file:./.ob-jupyter/a9ec50494a65a1ea77461baa3d1f032f672a52ba.png]]
...
#+RESULTS:
[[file:./.ob-jupyter/ee0fd4bbaf28f868aa3946bced024dbb7ffe4b2c.png]]
