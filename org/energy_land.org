...
for i in range(len(options['days'])):
    angle.append(angle_AB(-coefs_save[0][i], -coefs_save[1][i]))
print(angle)
angle = np.array(angle)
pkl_save(angle, 'angle_%s' % options['mouse'], path=".")
#+end_src
...
#+begin_src ipython
plt.plot(options['days'], angle, '-o')
plt.xlabel('Day')
plt.ylabel('Angle A/B vs Go/noGo (°)')
...
#+begin_src ipython
  ...
  for i, day in enumerate(options['days']):
      ...
      minima = find_multiple_minima_from_values(theta, circcvl(energy_day[i][1:]*100, windowSize), num_minima=2,
...
#+RESULTS:
:RESULTS:
: [[4.173280494589798, 101.26473261614233], [347.80430351104445, 92.73363783092167], [2.956554340277048, 173.40114635282183], [55.47472702914557], [23.603375399570947, 232.75914074072455]]
# [goto error]
: ---------------------------------------------------------------------------
