#+STARTUP: fold
#+PROPERTY: header-args:ipython :results both :exports both :async yes :session calcium :kernel dual
...
#+begin_src ipython
df_licks = pkl_load('df_licks', path="/storage/leon/dual_task/data/mice/")
#+end_src

#+RESULTS:
: loading from /storage/leon/dual_task/data/mice//df_licks.pkl
...
#+RESULTS:
[[file:./.ob-jupyter/97c43383b346dc55a593db83353862465cf154a5.png]]
...
#+RESULTS:
[[file:./.ob-jupyter/b9d4dfbd8d4a6f380af13f7cdc85f2f74063f301.png]]
...
#+RESULTS:
[[file:./.ob-jupyter/24c8034d353a46bc05d5a74f3c9587e2c148037c.png]]
...
: Cell In[39], line 6
:       3 options['deltaT'] = -2
:       5 options = set_options(**options)
: ----> 6 lick_sum = sum_epochs(lick_all, **options)
:       8 print(lick_sum.shape)
:
: NameError: name 'sum_epochs' is not defined
:END:
...
: NameError                                 Traceback (most recent call last)
: Cell In[40], line 1
: ----> 1 mean_licks = np.nanmean(lick_sum, 0)
:       2 sem_licks = stats.sem(lick_sum, axis=0, nan_policy='omit')
:       3 ci_licks = 1.96 * sem_licks
:
: NameError: name 'lick_sum' is not defined
:END:
...
Cell In[44], line 4
      2 df_licks = df_licks[df_licks.mouse!='JawsM12']
      3 df_all = df_all[df_all.laser==0]
----> 4 df_all['lick_times'] = df_licks['lick_times'].values # .reset_index(drop=True)

File ~/mambaforge/envs/dual/lib/python3.14/site-packages/pandas/core/frame.py:4322, in DataFrame.__setitem__(self, key, value)
   4319     self._setitem_array([key], value)
   4320 else:
   4321     # set column
-> 4322     self._set_item(key, value)

File ~/mambaforge/envs/dual/lib/python3.14/site-packages/pandas/core/frame.py:4535, in DataFrame._set_item(self, key, value)
...
File ~/mambaforge/envs/dual/lib/python3.14/site-packages/pandas/core/frame.py:5288, in DataFrame._sanitize_column(self, value)
...
-> 5288     com.require_length_match(value, self.index)
   ...
   (...)   5295     # TODO: Remove kludge in sanitize_array for string mode when enforcing
   5296     # this deprecation

File ~/mambaforge/envs/dual/lib/python3.14/site-packages/pandas/core/common.py:573, in require_length_match(data, index)
    569 """
...
ValueError: Length of values (4128) does not match length of index (5088)
#+end_example
:END:
...
File ~/mambaforge/envs/dual/lib/python3.14/site-packages/pandas/core/indexes/base.py:3812, in Index.get_loc(self, key)
   3811 try:
...
File ~/mambaforge/envs/dual/lib/python3.14/site-packages/pandas/core/frame.py:4113, in DataFrame.__getitem__(self, key)
...
File ~/mambaforge/envs/dual/lib/python3.14/site-packages/pandas/core/indexes/base.py:3819, in Index.get_loc(self, key)
...
#+RESULTS:
[[file:./.ob-jupyter/31bc3c681a9e8ed8c3e45c55d0a09e2fecbcb227.png]]
...
File ~/mambaforge/envs/dual/lib/python3.14/site-packages/seaborn/relational.py:485, in lineplot(data, x, y, hue, size, style, units, weights, palette, hue_order, hue_norm, sizes, size_order, size_norm, dashes, markers, style_order, estimator, errorbar, n_boot, seed, orient, sort, err_style, err_kws, legend, ci, ax, **kwargs)
    471 def lineplot(
    472     data=None, *,
    473     x=None, y=None, hue=None, size=None, style=None, units=None, weights=None,
   (...)    481
...
--> 485     p = _LinePlotter(
    ...
    489         ),
    490         estimator=estimator, n_boot=n_boot, seed=seed, errorbar=errorbar,
    491         sort=sort, orient=orient, err_style=err_style, err_kws=err_kws,
    492         legend=legend,
    493     )
    495     p.map_hue(palette=palette, order=hue_order, norm=hue_norm)
    496     p.map_size(sizes=sizes, order=size_order, norm=size_norm)

File ~/mambaforge/envs/dual/lib/python3.14/site-packages/seaborn/relational.py:216, in _LinePlotter.__init__(self, data, variables, estimator, n_boot, seed, errorbar, sort, orient, err_style, err_kws, legend)
    202 def __init__(
    203     self, *,
    204     data=None, variables={},
   (...)    210     # the kind of plot to draw, but for the time being we need to set
    211     # this information so the SizeMapping can use it
    212     self._default_size_range = (
    213         np.r_[.5, 2] * mpl.rcParams["lines.linewidth"]
    214     )
--> 216     super().__init__(data=data, variables=variables)
    218     self.estimator = estimator
    219     self.errorbar = errorbar

File ~/mambaforge/envs/dual/lib/python3.14/site-packages/seaborn/_base.py:634, in VectorPlotter.__init__(self, data, variables)
...
File ~/mambaforge/envs/dual/lib/python3.14/site-packages/seaborn/_base.py:679, in VectorPlotter.assign_variables(self, data, variables)
    674 else:
...
File ~/mambaforge/envs/dual/lib/python3.14/site-packages/seaborn/_core/data.py:58, in PlotData.__init__(self, data, variables)
     51 def __init__(
     52     self,
     53     data: DataSource,
...
File ~/mambaforge/envs/dual/lib/python3.14/site-packages/seaborn/_core/data.py:232, in PlotData._assign_variables(self, data, variables)
    230     else:
...
ValueError: Could not interpret value `licks_ED` for `y`. An entry with this name does not appear in `data`.
#+end_example
:END:
...
Cell In[51], line 6, in plot_licks(df, day, epoch, ax, title, y0, size, if_proba, ls)
      3 df_ = df.copy()
      4 colors = ['r', 'b', 'g']
...
File ~/mambaforge/envs/dual/lib/python3.14/site-packages/pandas/core/groupby/generic.py:1951, in DataFrameGroupBy.__getitem__(self, key)
...
File ~/mambaforge/envs/dual/lib/python3.14/site-packages/pandas/core/base.py:245, in SelectionMixin.__getitem__(self, key)
    243 else:
...
#+RESULTS:
:RESULTS:
[[file:./.ob-jupyter/62f77319b6f3d822eb0ccd54122d1f2d62701736.png]]
[[file:./.ob-jupyter/62f77319b6f3d822eb0ccd54122d1f2d62701736.png]]
[[file:./.ob-jupyter/da5ecde46c6cc4beb6271666f76a327026ecdb9c.png]]
[[file:./.ob-jupyter/0381bc8d7f29d757f5b024fd9b4b683d43b5646c.png]]
:END:
...
#+RESULTS:
[[file:./.ob-jupyter/f52eba422039b98c9d90d2c81d8e0d168d53f4a8.png]]
...
#+RESULTS:
:RESULTS:
: /home/leon/tmp/ipykernel_2540321/2623650266.py:10: UserWarning: The palette list has more values (2) than needed (1), which may not be intended.
:   sns.lineplot(data=df2, x='day', y='performance', hue='laser', marker='o', legend=1, palette=['k','y'], ax=ax[0])
: /home/leon/tmp/ipykernel_2540321/2623650266.py:18: UserWarning: The palette list has more values (2) than needed (1), which may not be intended.
:   sns.lineplot(data=df2, x='day', y='odr_perf', hue='laser', marker='o', legend=1, palette=['k', 'y',], ax=ax[1])
[[file:./.ob-jupyter/1dab3929beea51d0f02f8fc714874a590628b217.png]]
:END:
...
#+RESULTS:
:RESULTS:
: /home/leon/tmp/ipykernel_2540321/3753518863.py:11: UserWarning: The palette list has more values (2) than needed (1), which may not be intended.
:   sns.lineplot(data=df2, x='day', y='performance', hue='laser', marker='o', legend=1, palette=['k','y'], ax=ax[0])
: /home/leon/tmp/ipykernel_2540321/3753518863.py:19: UserWarning: The palette list has more values (2) than needed (1), which may not be intended.
:   sns.lineplot(data=df2, x='day', y='odr_perf', hue='laser', marker='o', legend=1, palette=['k', 'y',], ax=ax[1])
[[file:./.ob-jupyter/614493ddb6857e356b13b677bfbc7527ed5a11d4.png]]
:END:
