...
class bolasso(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, y):
        ...
        self.boots_coef_ = bootstrap_coefs(self.model_,
                                           X,
                                           y,
        ...
        _, self.p_val_ = ttest_ind(self.boots_coef_,
        ...
        try:
            ...
            print('coef', self.coef_.shape, 'lambda',
                  self.model_.named_steps['clf'].C_, 'non zero', np.sum(idx))
            ...
            print('coef', coef.shape, 'lambda', self.model_.named_steps['clf'].C_,
                  'non zero', np.sum(self.fs_idx_) )
        ...
        try:
            ...
            print('coefs', self.coef_.shape, 'non zero', np.sum(self.fs_idx_), 'scale', scale.shape, 'mean', mean.shape)
