...
def get_clf(**kwargs):
    ...
    if kwargs['in_fold'] == 'stratified':
        cv = StratifiedKFold(n_splits=kwargs['n_in'],
                             shuffle=True,
                             random_state=kwargs['random_state']) # outer cv loop for scoring

    if kwargs['in_fold'] == 'loo':
        cv = LeaveOneOut()

    if kwargs['in_fold'] == 'repeated':
        cv = RepeatedStratifiedKFold(n_splits=kwargs['n_in'],
    ...
    clf = LogisticRegressionCV(Cs=kwargs['Cs'],
    ...
    if 'bootstrap' in kwargs['clf_name']:
        pipe = BaggingClassifier(pipe, n_estimators=kwargs['n_boots'], n_jobs=kwargs['n_jobs'])
