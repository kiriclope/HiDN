...
if __name__ == '__main__':
    ...
    plt.plot(day_list+.1, perf_on[1], '-o', color=gv.pal[2], ms=1.5)

    plt.plot([0,6], [0.5, 0.5], '--k', alpha=0.5)

    plt.errorbar(day_list, perf_off[1], yerr=ci_off[1].T, fmt='none', alpha=1, color=gv.pal[1])
    plt.errorbar(day_list+.1, perf_on[1], yerr=ci_on[1].T, fmt='none', alpha=1, color=gv.pal[2])
